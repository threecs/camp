...
function sorter(nums) {
  //to loop through the array of numbers
  for (var i = 0; i < nums.length;i++) {

    //to select current numberg
    for (var j = 0; j < nums.length; j++){

      //to check if the number to the left is greater than the number to the right
      if(nums[j-1] > nums[j]){

        //to store the number on the left
        var tmp = nums[j-1];

        //to replace the number on the left with the number on the right
        nums[j-1] = nums[j];

        //to replace the number on the right with the stored number on the left
        nums[j] = tmp;
      }
    }
  }
...
// This way does the same thing, but JavaScript does most of the work for you:
